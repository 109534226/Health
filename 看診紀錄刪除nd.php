...
if (isset($_POST['id']) && !empty(trim($_POST['id']))) {
    ...
    if ($查詢結果 && mysqli_num_rows($查詢結果) > 0) {
        ...
        if (mysqli_stmt_execute($stmt_delete)) {
            echo "<script>
                    alert('刪除成功');
                    window.location.href = 'd_recordsfind.php?deleted=true&search=" . urlencode($searchTerm) . "';
                  </script>";
        } else {
            $錯誤訊息 = mysqli_error($link);
            echo "<script>
                    alert('刪除失敗: $錯誤訊息');
                    window.location.href = 'd_recordsfind.php?search=" . urlencode($searchTerm) . "';
                  </script>";
        }
    } else {
        echo "<script>
