...
<body>
    ...
    // 查詢當前頁碼的資料
    $查詢結果 = mysqli_query($link, "SELECT * FROM doctorshift WHERE hospital = '$指定醫院' LIMIT $起始位置, $每頁記錄數");
    if (!$查詢結果) {
        die("查詢失敗: " . mysqli_error($link));
    }
?>
        ...
        <tbody>
            <?php while ($資料列 = mysqli_fetch_assoc($查詢結果)): ?>
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($資料列['hospital']); ?></td>
                    <td><?php echo htmlspecialchars($資料列['shift_date']); ?></td>
                    <td><?php echo htmlspecialchars($資料列['shift_time']); ?></td>
                    <td><?php echo htmlspecialchars($資料列['doctor_name']); ?></td>
                    <td><?php echo htmlspecialchars($資料列['remarks']); ?></td>
                </tr>
            <?php endwhile; ?>
        </tbody>
    </table>
